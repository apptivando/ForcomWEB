...
const inputCls =
  "w-full bg-[#0D0D0F] border border-[#6A6A70] rounded-sm px-4 py-2.5 text-white text-[15px] focus:border-[#4A4A52] focus:ring-2 focus:ring-[#FF6A5C]/60 focus:ring-offset-1 focus:ring-offset-[#0D0D0F] focus:outline-none transition-colors";
...
export default function HeroSlideForm({ slide, slideCount, onSave, onClose }: Props) {
  ...
  return (
    <Modal
      ...
      subtitle={slide ? `Modificando: ${slide.badge_text}` : "Se agregará al final del carrusel"}
      // Un clic al costado no puede tirar un slide a medio escribir.
      ...
      footer={
        <div className="flex items-center justify-end gap-3">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2.5 border border-[#2A2A2E] text-[#B0B0B0] text-[15px] font-display font-semibold rounded-sm hover:text-white hover:border-[#8A8A8A] transition-colors"
          >
            Cancelar
          </button>
          <button
            ...
            className="px-6 py-2.5 bg-[#C41D16] text-white text-[15px] font-display font-bold tracking-wider uppercase rounded-sm hover:bg-[#E8231A] transition-colors disabled:opacity-50"
          >
    ...
    >
      <form id="hero-slide-form" onSubmit={handleSubmit} className="px-6 py-6 space-y-5">
          {/* Badge */}
          ...
          {/* Headline */}
          <div className="space-y-3 p-4 bg-[#0D0D0F] border border-[#6A6A70] rounded-sm">
            ...
            {(form.headline_line1 || form.headline_line2 || form.headline_accent) && (
              <div className="mt-2 px-4 py-3 bg-[#141416] border border-[#2A2A2E] rounded-sm">
                <p className="text-[12px] tracking-[0.15em] uppercase text-[#8A8A8A] mb-1">Vista previa</p>
                <p className="font-display font-extrabold text-2xl leading-tight text-white">
                  ...
                  {form.headline_accent && (
                    <>
                      {" "}
                      <br />
                      <span className="text-[#FF6A5C]">{form.headline_accent}</span>
                    </>
                  )}
                </p>
              </div>
          ...
          {/* Image */}
          <div className="space-y-4 p-4 bg-[#0D0D0F] border border-[#6A6A70] rounded-sm">
            <p className="text-xs font-display font-semibold tracking-[0.15em] uppercase text-[#8A8A8A]">
              Imagen del producto
            </p>
            <p className="text-[13px] text-[#8A8A8A]/70">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <button
                ...
                className="px-4 py-2 border border-[#2A2A2E] text-[#B0B0B0] text-[15px] font-display font-semibold rounded-sm hover:border-[#E8231A] hover:text-white transition-colors"
              >
                {imagePreview ? "Cambiar imagen" : "Subir imagen"}
              </button>
              {imageFile && (
                <span className="text-[13px] text-[#8A8A8A] truncate max-w-xs">{imageFile.name}</span>
              )}
            </div>
          ...
          <div className="flex items-center gap-3">
            <Toggle
              ...
              label="Slide activo en el carrusel"
            />
            <span className="text-[15px] text-[#B0B0B0] font-display">
              {form.active ? "Activo — se muestra en el carrusel" : "Inactivo — oculto en el sitio"}
            </span>
          </div>

          {formError && (
            <p className="text-[15px] text-[#FF6A5C] bg-[#E8231A]/10 border border-[#E8231A]/30 rounded-sm px-4 py-3">
              {formError}
            </p>
          )}
      </form>
